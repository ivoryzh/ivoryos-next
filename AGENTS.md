...
flowchart LR
    ...
    K1 --> K2
    K2 --> K3
    K3 -.->|MQTT heartbeat + task topics<br/>Core already speaks this| L1
    L1 --> L2
    K3 -->|share results, ask questions| C1
    H1 -.->|ROADMAP: store the introspected<br/>schema, not just install metadata| K3
```

The two dotted lines are the two open threads, in priority order:
1. **Cloud ↔ Core is not actually connected today.** Core speaks MQTT (`edge_server/ivoryos_edge/server.py`'s `setup_broker`/`heartbeat_loop`, publishing to `ivoryos/edge/{id}/heartbeat` and subscribing to `.../execute`). `cloud_frontend` has *two disconnected halves* of a client for this: an HTTP path (`api/edge/heartbeat`, `api/devices`, `src/lib/orchestrator.ts`) backed by an in-memory `Map` that real Core never calls, and an MQTT path (`daemon.js`) that only **subscribes** and never publishes a dispatched task — and isn't started by any `package.json` script. `api/cloud-workflows/status` reads `daemon.js`'s state; `api/cloud-workflows/runs` (dispatch) writes to the unrelated orchestrator Map. End to end: a real Core instance would never appear in Cloud's device list, and a workflow "dispatched" from Cloud would never reach it. There is also no auth on any of these routes and no persistent device registry (state is an in-memory Map plus JSON files in `os.tmpdir()`, wiped on restart). If you're asked to work on Cloud, this is almost certainly the first real problem to solve — pick one transport (MQTT, since that's what Core already implements) and make the Next.js API routes and `daemon.js` share one real, persistent store.
