...
1. **Cloud ↔ Core sync (read path) is wired; dispatch (write path) is not — yet.** The transport is settled: MQTT / AWS IoT Core only, chosen because Core already fully implements it (`broker.py`'s `LocalMQTTBroker`/`AWSIoTBroker`, configured entirely from a base64 `CLOUD_TOKEN` env var — no self-hosted broker required, which matters since Cloud is meant to be a zero-setup SaaS). The old HTTP long-poll path (`api/edge/heartbeat`, `api/edge/complete`) has been deleted; it was never called by a real Core instance anyway.
   ...
   - **UNRESOLVED, live bug (as of 2026-09-09): AWS IoT connections intermittently self-disconnect and reconnect in rapid bursts** — `_on_disconnect` (`broker.py`) fires with `is_disconnect_packet_from_server=False` and reason `'Unspecified error'`, over and over, sometimes within 1-3 seconds of a successful CONNACK. Ruled out so far, each with a real isolated test: (1) not a client-ID collision — reproduced with zero competing connections; (2) not specific to one Thing/certificate — reproduced on a completely fresh, never-before-connected Thing; (3) not the MQTT5-vs-3.1.1 protocol version — forcing `protocol=mqtt.MQTTv311` reduced but didn't eliminate it. Leading hypothesis, not confirmed: AWS IoT-side connection-rate throttling from the sheer volume of connect attempts made while diagnosing this (15-20+ MQTT connections and several `CreateThing`/`CreateKeysAndCertificate` calls inside about an hour, from one source IP/account) — every additional test connection is also a potential contributor, so diagnosing this further needs a cooldown period before more probing, not more of it. Practical consequence while unresolved: `schema`/`sequences` can take longer to reach Supabase than expected (mitigated below — they now retry every ~60s instead of firing once) and `devices.status` may show `offline` even when a device is genuinely connected, since each reconnect burst re-fires the (unretained, since the earlier fix) Last Will. If you pick this up: do NOT rapid-fire more test connections to investigate — that's indistinguishable from being part of the problem; wait first, and check AWS IoT's CloudWatch connection/disconnect metrics for this account if trying to confirm the throttling theory.
