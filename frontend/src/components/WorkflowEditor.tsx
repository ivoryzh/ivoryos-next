...
import { GripVertical, Trash2, Settings2, ChevronDown, ChevronUp, AlertTriangle, Eye, EyeOff, Info, PanelRightClose, PanelRightOpen, ChevronsDownUp, ChevronsUpDown, ChevronRight, Search } from 'lucide-react';
                  ...
                  const checkRequiredParams = (schemaObj: any, prefix: string = '') => {
                      for (const p of Object.keys(schemaObj)) {
                          ...
                          if (pData?.is_object && pData?.fields) {
                              ...
                              // Allow dynamic variables (strings starting with #) to pass
                                              ...
                                              return (
                                                <div key={paramKey} className="relative flex items-center">
                                                  <input
                                                    ...
                                                    className={`w-32 border rounded px-2 py-1 text-xs focus:outline-none ${hashInvalid ? 'border-red-400 dark:border-red-500 focus:border-red-500' : flowInputClass}`}
                                                  />
                                                </div>
                                              )
                                            })}
    ...
    <DragDropContext onDragEnd={onDragEnd}>
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-72 bg-white dark:bg-[#1a1a1a] flex flex-col border-r border-gray-200 dark:border-white/10 shrink-0 z-10">
          <div className="h-16 px-4 flex items-center border-b border-gray-200 dark:border-white/10 bg-gray-50/60 dark:bg-black/10 shrink-0">
